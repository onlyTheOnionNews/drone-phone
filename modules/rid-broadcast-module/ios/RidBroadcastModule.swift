...
class BluetoothDelegateHandler: NSObject, CBPeripheralManagerDelegate {

    // MARK: - Proxy Closures
    // These closures will be set by the RidBroadcastModule to receive callbacks.
    var onStateUpdate: ((CBPeripheralManager) -> Void)?
    var onAdvertisingStart: ((CBPeripheralManager, Error?) -> Void)?

    // MARK: - CBPeripheralManagerDelegate Methods

    public func peripheralManagerDidUpdateState(_ peripheral: CBPeripheralManager) {
        // Execute the closure to notify the main module of the state change.
        onStateUpdate?(peripheral)
    }

    public func peripheralManagerDidStartAdvertising(
        _ peripheral: CBPeripheralManager, error: Error?
    ) {
        // Execute the closure to notify the main module about the advertising result.
        onAdvertisingStart?(peripheral, error)
    }
}

public class RidBroadcastModule: Module {

    // Private properties to manage the Bluetooth state.
    private var peripheralManager: CBPeripheralManager?
    private var broadcastTimer: Timer?
    private var allDroneData: [String: Any]?
    private var delegateHandler: BluetoothDelegateHandler?
    ...
    public func definition() -> ModuleDefinition {
        ...
        OnCreate {
            let handler = BluetoothDelegateHandler()

            handler.onStateUpdate = { [weak self] peripheral in
                self?.handlePeripheralStateUpdate(peripheral)
            }

            handler.onAdvertisingStart = { [weak self] peripheral, error in
                self?.handleAdvertisingStart(peripheral, error: error)
            }

            self.delegateHandler = handler
            let queue = DispatchQueue(label: "com.dronephone.rid.bluetooth")
            peripheralManager = CBPeripheralManager(delegate: self.delegateHandler, queue: queue)
        }
         ...
         */
        AsyncFunction("startBroadcast") { (data: [String: Any], promise: Promise) in
            guard let manager = peripheralManager, manager.state == .poweredOn else {
                let error = NSError(
                    domain: "RidBroadcastModule",
                    code: 1,
            ...
            DispatchQueue.main.async {
                self.stopBroadcastInternal()
                self.broadcastTimer = Timer.scheduledTimer(withTimeInterval: 1.0, repeats: true) {
                    [weak self] _ in
                    self?.updateBroadcastFrame()
                }
                // The first frame update will trigger the first advertising attempt.
                self.updateBroadcastFrame()
            }
        }
    ...
    private func stopBroadcastInternal() {
        ...
        if let promise = self.startPromise {
            let error = NSError(
                domain: "RidBroadcastModule",
                code: 2,
                userInfo: [
                    NSLocalizedDescriptionKey: "The broadcast was stopped before it could start."
                ]
            )
    ...
    // MODIFIED: No longer needs a do-catch block as payload creation does not throw.
    private func updateBroadcastFrame() {
        guard let data = allDroneData, let manager = peripheralManager, manager.state == .poweredOn
        else { return }
    ...
    private func createAuthenticationPayload(data: [String: Any]) -> Data {
        guard let privateKeyString = data["privateKey"] as? String,
            ...
            let uasId = data["uasId"] as? String
        else {
    ...
    private func handlePeripheralStateUpdate(_ peripheral: CBPeripheralManager) {
        if peripheral.state == .poweredOff {
            stopBroadcastInternal()
        }
        print(
            "[RidBroadcastModule] Peripheral manager state changed to: \(peripheral.state.rawValue)"
        )
    }

    private func handleAdvertisingStart(_ peripheral: CBPeripheralManager, error: Error?) {
        if let error = error {
            print("[RidBroadcastModule] Failed to start advertising: \(error.localizedDescription)")
        } else {
            print("[RidBroadcastModule] Started advertising successfully.")
        }
    }
